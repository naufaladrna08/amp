...
void RefreshInfo(ImFileDialogInfo* dialogInfo)
{
  ...
  for (const std::filesystem::directory_entry& entry : std::filesystem::directory_iterator(dialogInfo->directoryPath)) {
    if (entry.path().filename().string().find('.') == 0 && !(dialogInfo->flags & ImGuiFileDialogGFlag_ShowHiddenFiles)) {
      continue;
    }
    ...
    if (entry.is_directory()) {
      ...
      if (dialogInfo->allowedExtensions.empty()) {
        dialogInfo->currentFiles.push_back(entry);
      } else {
        for (const std::string& extension : dialogInfo->allowedExtensions) {
...
bool ImGui::FileDialog(bool* open, ImFileDialogInfo* dialogInfo) {
  ...
  bool complete = false;
  std::string path = dialogInfo->directoryPath.string();
  ...
  if (ImGui::Begin(dialogInfo->title.c_str(), open)) {
    ...
    for (size_t i = 0; i < directories->size(); ++i) {
      ...
      std::string directoryNameStr = ICON_FA_FOLDER + std::string(" ") +  directoryName.string();

      if (ImGui::Selectable(directoryNameStr.c_str(), dialogInfo->currentIndex == index, ImGuiSelectableFlags_AllowDoubleClick, ImVec2(ImGui::GetWindowContentRegionMax().x, 0))) {
        dialogInfo->currentIndex = index;
    ...
    for (size_t i = 0; i < files->size(); ++i) {
      ...
      std::string fileNameStr = ICON_FA_FILE + std::string(" ") + fileName.string();

      if (ImGui::Selectable(fileNameStr.c_str(), dialogInfo->currentIndex == index, ImGuiSelectableFlags_AllowDoubleClick, ImVec2(ImGui::GetWindowContentRegionMax().x, 0))) {
        dialogInfo->currentIndex = index;
